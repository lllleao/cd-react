...
const Contact = () => {
    ...
    async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
        ...
        const { nameIsValid, messageIsValid, emailIsValid, numberIsValid } = authentic(emailUser, numEmail, name, text)


        emailIsValid ? setInputErrorEmail(false)
            : setInputErrorEmail(true)
    ...
    return (
        <ContactContainer id="contact-us" className="contact-us container">
            <div className="pix">
                ...
                <input className={`input-pix ${isCopied && 'input-pix--is-copied'}`} disabled type="text" value="cidadeclipse@gmail.com" />
                <a onClick={handleClickPix} className="button-pix">COPIAR PIX <svg enableBackground="new 0 0 512 512" xmlSpace="preserve"
                    viewBox="0 0 6.35 6.35" y="0" x="0" height="20" width="20"
                    xmlnsXlink="http://www.w3.org/1999/xlink" version="1.1" xmlns="http://www.w3.org/2000/svg"
                    className="clipboard">
                    <g>
            ...
            <div className="contact-email">
                <div className={`success ${successForm && 'success-is-active'}`}>{messageSuccess ? 'Carregando...' : 'Enviado com sucesso'}</div>
                <form onSubmit={(e) => handleSubmit(e)} ref={formRef} className="contact-email__form">
                    ...
                    <div className="text-field">
                        <textarea
                            ...
                            placeholder="Olá! Achei as edições incriveis e gostaria de colaborar."></textarea>
                    </div>
                    <button className="button-pix button-contact" type="submit">Enviar</button>
                </form>
            </div>
        </ContactContainer>
    )
